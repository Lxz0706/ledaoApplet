<template>
	<view class="fileInfoContainer">
		<uni-forms ref="formDataFile" :modelValue="formDataFile" :rules="rules">
			...
			<!-- 			 <uni-forms-item label="documentStatu" name="documentStatu" v-if="false">
			...
			<view v-if="documentType == 0">
				...
				<uni-forms-item label="资产编号" name="assetNumber">
					<uni-easyinput type="text" :disabled="showInfo || !documentAdmin"
						v-model="formDataFile.assetNumber" />
				</uni-forms-item>
				<uni-forms-item label="合同编号" name="contractNo">
					<uni-easyinput type="text" :disabled="showInfo || !documentAdmin"
						v-model="formDataFile.contractNo" />
				</uni-forms-item>
			</view>
			<!-- end -->
			<!-- 日常经营类 -->
			<view v-if="documentType !== '0'">
				...
				<uni-forms-item label="种类"
					:name="documentType == 1 ? 'dailyDocumentTypeLable' : 'dailyDocumentTypeContractLable'">
					<uni-easyinput v-if="documentType == 1" type="text" disabled
						v-model="formDataFile.dailyDocumentTypeLable" />
			...
			<uni-forms-item required label="附件类型" name="fileTypeLable">
				<uni-easyinput v-if="showInfo || !documentAdmin" type="text" disabled
					v-model="formDataFile.fileTypeLable" />
				<picker v-else :value="formDataFile.fileType" :range="rangeFileType" @change="bindPickerChangeFileType">
					<view class="uni-input select-picker">
						{{formDataFile.fileType == '' || formDataFile.fileTypeLable == '' ? "请选择" : formDataFile.fileTypeLable}}
					</view>
				</picker>
			</uni-forms-item>
			<uni-forms-item required v-if="showFileScan" label="扫描件类型" name="fileScanTypeLable">
				...
				<uni-easyinput v-if="showInfo|| !documentAdmin " type="text" disabled
					v-model="formDataFile.fileScanTypeLable" />
				<picker v-else :value="formDataFile.fileScanType" :range="rangeFileScanType"
					@change="bindPickerChangeFileScanType">
					<view class="uni-input select-picker">
						{{formDataFile.fileScanType == '' && formDataFile.fileScanTypeLable == '' ? "请选择" : formDataFile.fileScanTypeLable}}
					</view>
				</picker>
			</uni-forms-item>
			...
			<view v-if="documentAdmin">
				<uni-forms-item required label="档案状态" name="documentStatuLable">
					...
					<picker v-else :value="formDataFile.fileGetType" :range="rangeDocumentStatu"
						@change="bindPickerChangeDocumentStatu">
						<view class="uni-input select-picker">
							{{formDataFile.documentStatu == '' && formDataFile.documentStatuLable == '' ? "请选择" : formDataFile.documentStatuLable}}
						</view>
					</picker>
				</uni-forms-item>
				<uni-forms-item label="柜号" name="cabinetNo">
					<uni-easyinput type="text" :disabled="showInfo || !documentAdmin"
						v-model="formDataFile.cabinetNo" />
				</uni-forms-item>
				...
				<uni-forms-item label="档案类型" name="documentGetTypeLable">
					...
					<picker v-else :value="formDataFile.documentGetType" :range="rangeDocumentGetType"
						@change="bindPickerChangeDocumentGetType">
						<view class="uni-input select-picker">
							{{formDataFile.documentGetType == '' && formDataFile.documentGetTypeLable == '' ? "请选择" : formDataFile.documentGetTypeLable}}
						</view>
					</picker>
				</uni-forms-item>
				<uni-forms-item label="文件类型" name="fileGetTypeLable">
					...
					<picker v-else :value="formDataFile.fileGetType" :range="rangeFileGetType"
						@change="bindPickerChangeFileGetType">
						<view class="uni-input select-picker">
							{{formDataFile.fileGetType == '' && formDataFile.fileGetTypeLable == '' ? "请选择" : formDataFile.fileGetTypeLable}}
						</view>
					</picker>
				</uni-forms-item>
				<uni-forms-item label="档案级别" name="documentLevelLable">
					...
					<picker v-else :value="formDataFile.documentLevel" :range="rangeDocumentLevel"
						@change="bindPickerChangeDocumentLevel">
						<view class="uni-input select-picker">
							{{formDataFile.documentLevel == '' && formDataFile.documentLevelLable == '' ? "请选择" : formDataFile.documentLevelLable}}
						</view>
					</picker>
				</uni-forms-item>
			</view>
