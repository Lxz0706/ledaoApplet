	...
	<view class="container">
		<view class="flow-container">
			<uni-forms ref="form" :modelValue="formData" :rules="rules" label-width="100">
				...
				<view class="out" v-if="formData.applyType == 1">
					...
					<uni-forms-item name="isOut" label="已出库">
						<picker v-if="documentAdmin && statu == 0" :value="formData.isOut" :range="rangeIsOut"
							@change="bindPickerChangeIsOut">
			...
			<view class="footer">
				...
				<button v-if="!(statu == 0) && documentAdmin && approveStatuFlag" type="primary"
					@click="successClick">确定</button>
			</view>
		</view>
		<!-- 弹框 -->
		<view class="modal">
			<modal v-if="showPop" title="审批意见" confirm-text="确定" cancel-text="取消" @cancel="cancelPop"
				@confirm="confirmPop">
	...
	export default {
		...
		methods: {
			...
			fileInfoClick(e, val) {
				...
				uni.navigateTo({
					url: '../../common/fileInfo/file?item=' + fileObjectVal + '&documentType=' + this
						.documentType + '&applyType=' + this.applyType + '&statu=' + this.statu + '&companyName=' +
						this.formData.companyNameLab + '&projectName=' + this.formData.projectName
				})
			}
		}
	}
