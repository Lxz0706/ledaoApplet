<template>
	<view class="addCurstomer">
		<view class="bgcolor">客户管理
			<image class="bgcolor-icon" :src="cusIcon + '.svg'" @click="curstomerIcon">
				<div style="font-size: 12px;">名片扫描</div>
			</image>
		</view>
		<uni-forms ref="form" class="cur-forms" :modelValue="curstomerForm" :rules="rules" label-width="75">
			<view>
				...
				<uni-forms-item required name="customerLable" label="客户标签">
					<uni-data-checkbox :wrap="true" multiple v-model="curstomerForm.customerLable"
						:localdata="customerLables" />
				</uni-forms-item>
				...
				<uni-forms-item label="共享人" name="shareUserName">
					...
					<view class="choose_css" @click="showSharerProRight()">
						...
						<block v-else v-for="(item,index) in curstomerForm.shareUserName" :key="index">{{item + ','}}
						</block>
		...
		<uni-drawer ref="showProRight" mode="right" :mask-click="true" :width="300">
			<scroll-view style="height: 100%;" scroll-y="true">
				<uni-row class="demo-uni-row" style="margin-top: 20rpx; margin-bottom: 20rpx;">
					...
					<uni-col :span="7">
						<view class="menu_text_close" @click="closeProDrawer">
		...
		<uni-drawer ref="showSharerProRight" mode="right" :mask-click="true" :width="300">
			<scroll-view style="height: 100%;" scroll-y="true">
				<uni-row class="demo-uni-row" style="margin-top: 20rpx; margin-bottom: 20rpx;">
					<uni-col :span="6">
						<view class="menu_text" @click="closeProDrawerShare">
							<uni-icons type="back" size="30">返回</uni-icons>
						</view>
					</uni-col>
					<uni-col :span="18">
				...
				<checkbox-group @change="checkChange">
					<uni-row class="demo-uni-row" v-for="(item,index) in applyUserNameList" :key="index">
						...
						<uni-col :span="4">
							<checkbox :value="item.userName" :checked="false" color="#FFCC33"
				...
				ocrList.map(item => {
					...
					if (item.Name == '姓名') {
						...
						if (item.Value.length > 11) {
							this.curstomerForm.contactNumber = item.Value.substring(item.Value.length - 11, item
								.Value.length);
						} else {
						...
						if (this.curstomerForm.contactAddress.indexOf("区") != -1) {
							if (this.curstomerForm.contactAddress.indexOf("市") != -1) {
								this.curstomerForm.county = this.curstomerForm.contactAddress.split('市')[1].split(
									'区')[0] + '区'
							} else {
			...
			submit() {
				...
				this.$refs.form.validate().then(res => {
					...
					this.$request("/system/customer/checkPhoneUnique", "POST", {
						...
						if (resPhone == 'login' || (resPhone.code == 500 && resPhone.msg.includes(
								"Authentication"))) {
							uni.setStorageSync('loginSuccess', false)
							setTimeout(function() {
								uni.showToast({
					...
					this.$request("/system/customer/checkWeChatNumberUnique", "POST", {
						...
						if (resWeChat == 'login' || (resWeChat.code == 500 && resWeChat.msg.includes(
								"Authentication"))) {
							uni.setStorageSync('loginSuccess', false)
							setTimeout(function() {
								uni.showToast({
					...
					setTimeout(() => {
						if (flagPhone && flagWeChat) {
							...
							this.$request("/system/customer/add", "POST", {
								...
								if (resCur == 'login' || (resCur.code == 500 && resCur.msg
